                            ...
                            <div class="min-w-[16rem] flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @if($activeUntil = $listing->currentPromotion()?->activeUntilPhrase())
                                        <span class="inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800 ring-1 ring-inset ring-amber-200">{{ $activeUntil }}</span>
                                    @elseif($listing->ownerPromotion()?->status === \App\Enums\ListingPromotionStatus::Pending)
                                        <span class="inline-flex rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-700 ring-1 ring-inset ring-gray-200">{{ $listing->ownerPromotion()->memberStatusLabel() }}</span>
                                    @endif
                                </div>
                                ...
                                <p class="mt-0.5 break-words text-sm text-gray-500">{{ $listing->location_name }}</p>
                            </div>
                            <div class="flex flex-wrap gap-2 sm:ml-auto">
                                ...
                                @can('markSold', $listing)
                                    <form method="POST" action="{{ route('market.sold', $listing) }}">
                                        @csrf
                                        <button type="submit" class="btn-secondary px-3 py-1.5 text-sm whitespace-nowrap">Mark sold</button>
                                    </form>
                                @endcan
                                @can('markExchanged', $listing)
                                    <form method="POST" action="{{ route('market.exchanged', $listing) }}">
                                        @csrf
                                        <button type="submit" class="btn-secondary px-3 py-1.5 text-sm whitespace-nowrap">Mark exchanged</button>
                                    </form>
                                @endcan
                                @can('markDonated', $listing)
                                    <form method="POST" action="{{ route('market.donated', $listing) }}">
                                        @csrf
                                        <button type="submit" class="btn-secondary px-3 py-1.5 text-sm whitespace-nowrap">Mark donated</button>
                                    </form>
                                @endcan
                                @can('close', $listing)
                                    <form method="POST" action="{{ route('market.close', $listing) }}">
                                        @csrf
                                        <button type="submit" class="btn-secondary px-3 py-1.5 text-sm whitespace-nowrap">Close</button>
                                    </form>
                                @endcan
                                @can('delete', $listing)
                                    <form
                                    ...
                                    >
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-secondary px-3 py-1.5 text-sm whitespace-nowrap text-red-700">Delete</button>
                                    </form>
                                @endcan
                            </div>
                        </div>
