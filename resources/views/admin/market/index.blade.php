<x-app-layout>
    ...
    <div class="py-8" x-data="adminMarketQueue">
        <div class="mx-auto max-w-6xl sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex flex-wrap gap-2 text-sm" data-admin-market-tabs>
                @foreach(['all' => 'All', 'pending' => 'Pending', 'published' => 'Published', 'rejected' => 'Rejected', 'reported' => 'Reported'] as $value => $label)
                    <a
                        href="{{ route('admin.market.index', array_filter(['status' => $value, 'q' => $search])) }}"
                        data-admin-market-filter="{{ $value }}"
                        class="rounded-full px-3 py-1 {{ $status === $value ? 'bg-forest-800 text-white' : 'bg-white text-gray-700 ring-1 ring-gray-200' }}"
                    >{{ $label }}</a>
                @endforeach
            </div>

            <form method="GET" action="{{ route('admin.market.index') }}" class="mb-4 flex flex-wrap gap-2" data-admin-market-search>
            ...
            <div class="relative">
                ...
                >Loading…</div>
                <div x-ref="results">
                    @include('admin.market.partials.results')
                </div>
            </div>
        </div>
    </div>
